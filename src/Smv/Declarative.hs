-- | Desugaring of ASSIGN-style SMV modules into declarative form.
module Smv.Declarative where
...
-- | Desugar a module's ASSIGN entries into INIT and TRANS.
transformDeclarative :: Monad m => PackedPmodule -> m PackedPmodule
transformDeclarative pmodule@(PackedPmodule name vars defines init invar trans assigns ltlspec justice) = do
    ...
    return (PackedPmodule name vars' defines init' invar trans' noPackedPassigns ltlspec justice)

-- | Desugar ASSIGN entries and fold INVAR into INIT and TRANS.
...
-- | Convert ASSIGN entries into init and next expressions.
assignsToExprs :: PackedPassigns -> (Pexpr,Pexpr)
assignsToExprs (PackedPassigns i n) = (initsToExpr i,nextsToExpr n)

-- | Convert init assignments into a conjoined expression.
...
-- | Convert next assignments into a conjoined expression.
...
-- | Pad a variable's domain to a power-of-two-sized enum.
...
transformBooleanDomains (PackedPmodule name vars defines init invar trans assigns ltlspec justice) = do
    (vars',vars_invar') <- State.runStateT (K.mapWithKeyM mkBooleanVar vars) ptrue
    return (PackedPmodule name vars' defines init (pand invar vars_invar') trans assigns ltlspec justice)
