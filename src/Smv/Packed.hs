-- | A packed representation of an SMV module.
...
-- | A Map-based SMV module: fields keyed by identifier rather than item lists.
...
-- | Remove a module's LTLSPEC.
dropLTLSpec :: PackedPmodule -> PackedPmodule
dropLTLSpec p = p { p_ltlspec = Nothing }

-- | Conjoin an expression into a module's LTLSPEC (or INVAR, for G-formulas).
...
-- | ASSIGN entries split into init and next maps.
data PackedPassigns = PackedPassigns { p_inits :: PackedPdefs, p_nexts :: PackedPdefs }
    deriving (Eq,Ord,Show)

-- | A map from identifiers to their defining expressions.
type PackedPdefs = Map Pident Pexpr

-- | A map from identifiers to their declared types.
...
packPmodule (Pmodule name defs) = State.execStateT (mapM_ packPitem defs) st
    ...
    packPitem (L.Located l d) = case d of
        ...
        Piassign as -> do
            ...
            State.modify $ \st -> st { p_assigns = assigns' }
        Pijustice e -> State.modify $ \st -> st { p_justice = e : p_justice st }

-- | Convert a packed module back into a 'Pmodule'.
unpackPmodule :: Monad m => PackedPmodule -> m Pmodule
unpackPmodule (PackedPmodule name vars defines init invar trans assigns ltlspec justice) = do
    ...
    let justice' = map (\e -> L.dummyLoc $ Pijustice e) justice
...
-- | Pack ASSIGN entries into an existing 'PackedPassigns'.
...
-- | Unpack a 'PackedPassigns' into ASSIGN entries.
...
-- | The identifiers a module declares (vars and defines).
...
-- | Merge extra DEFINE entries into a module.
