...
--pbAtLeast :: forall w. (Real w) => IntMap w -> w -> BDD
...
--
-- | Pseudo-boolean constraint, /w1*x1 + w2*x2 + … ≤ k/.
--pbAtMost :: forall w. (Real w) => IntMap w -> w -> BDD
...
--pbExactly :: forall w. ( Real w) => IntMap w -> w -> BDD
...
--pbExactlyIntegral :: forall w. ( Real w, Integral w) => IntMap w -> w -> BDD
...
--existsUniqueSet ::  IntSet -> BDD -> BDD
...
--restrictSet ::  IntMap Bool -> BDD -> BDD
...
--substSet ::  IntMap (BDD) -> BDD -> BDD
--substSet s m = runST $ do
--  supportOp <- mkSupportOp
--
--  h <- C.newSized defaultTableSize
--  let -- f :: IntMap (BDD) -> [(IntMap Bool, BDD)] -> IntMap (BDD) -> ST _ (BDD)
...
--
--  where
--    propagateFixed :: IntMap (BDD) -> [(IntMap Bool, BDD)] -> (IntMap (BDD), [(IntMap Bool, BDD)])
...
findSatCompleteM is bdd = expandPartial =<< findSatM bdd
    where
...
--
--{-# SPECIALIZE countSat :: IntSet -> BDD -> Int #-}
--{-# SPECIALIZE countSat :: IntSet -> BDD -> Integer #-}
--{-# SPECIALIZE countSat :: IntSet -> BDD -> Natural #-}
...
-- >>> maxBound :: Int64
-- 9223372036854775807
--countSat :: forall b. ( Num b, Bits b, HasCallStack) => IntSet -> BDD -> b
...
-- .
-- #if MIN_VERSION_mwc_random(0,15,0)
-- uniformSatM :: forall g m. ( StatefulGen g m, HasCallStack) => IntSet -> BDD -> g -> m (IntMap Bool)
-- #else
-- uniformSatM :: forall m. ( PrimMonad m, HasCallStack) => IntSet -> BDD -> Gen (PrimState m) -> m (IntMap Bool)
-- #endif
